...
export type MirageXServerConfig = {
  mirage: {
    url: string;
    port: string;
    serverId: string;
...
export class MirageXServer {
  ...
  constructor(app: () => JSX.Element, config: MirageXServerConfig) {
    ...
    if (this.config.auth.defaultAuthenticationToken) {
      console.info(
        "use defaultAuthenticationToken",
        this.config.auth.defaultAuthenticationToken,
      );
    }
  ...
  private websocketController = (ws: WebSocket, req: http.IncomingMessage) => {
    ...
    const connection = new Connection({
      ...
      app: this.app,
      platformApiUrl: this.config.platform.api.url,
      defaultAuthenticationToken: this.config.auth.defaultAuthenticationToken,
      eventCountSolver: this.config.mirage.eventCountSolver,
    });
  ...
  private monitConnectionsController(
    ...
    res.send(
      `<html><body>${contents.map(
        (c) =>
          `<div>${c.id} ${c.ownerId} ${c.isLoggedIn} ${c.eventCount}</div>`,
      )}</body></html>`,
    );
  }
      ...
      if (func) {
        try {
          ...
          if (process.env.NODE_ENV !== "production") {
            setTimeout(() => {
              try {
                const result = func({ userId, lang }, ...args);
                if (isPromise(result)) {
                  result.catch((e) => {
                    console.error("catch interactionEvent error", e);
                  });
                }
              } catch (e) {
                console.error(e);
